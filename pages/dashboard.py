...
with tab_activity:
    ...
    if appr_codes:
        ...
        # --- Robot share (avg 2021–2025) scatter right after surgical approaches ---
            try:
                ...
                scope_rs = st.radio(
                    ...
                    horizontal=True,
                    index=0,
                    key=f"robot_share_scope_after_appr_{selected_hospital_id}"
                )
                ...
                reg_val_rs = _extract_region_from_details(selected_hospital_details)
                ids_all_rs = establishments['id'].astype(str).unique().tolist() if 'id' in establishments.columns else []
                ids_reg_rs = (
                    establishments[establishments.get('lib_reg', establishments.get('region', '')).astype(str).str.strip() == str(reg_val_rs)]['id'].astype(str).unique().tolist()
                    if reg_val_rs is not None and 'id' in establishments.columns else []
                )
                status_val_rs = str(selected_hospital_details.get('statut', '')).strip()
                ids_status_rs = (
                    establishments[establishments.get('statut','').astype(str).str.strip() == status_val_rs]['id'].astype(str).unique().tolist()
                    if 'statut' in establishments.columns else []
                )

                if scope_rs == "Regional":
                    ids_scope = ids_reg_rs
                elif scope_rs == "Same status":
                    ids_scope = ids_status_rs
                else:
                    ids_scope = ids_all_rs

                # Aggregate 2021–2025 per hospital
                df = annual[(annual.get('annee') >= 2021) & (annual.get('annee') <= 2025)].copy()
                if ids_scope:
                    df = df[df['id'].astype(str).isin([str(i) for i in ids_scope])]
                if df.empty:
                    st.info('No data to compute robot share for the selected scope.')
                else:
                    cols = [c for c in ['ROB','COE','LAP'] if c in df.columns]
                    if not cols:
                        st.info('Approach columns not available.')
                    else:
                        grouped = df.groupby('id', as_index=False).agg({
                            **{c: 'sum' for c in cols},
                            **({'total_procedures_year': 'mean'} if 'total_procedures_year' in df.columns else {})
                        })
                        grouped = grouped.rename(columns={'id':'hid'})
                        grouped['_approach_tot'] = grouped[cols].sum(axis=1)
                        grouped = grouped[grouped['_approach_tot'] > 0]
                        if 'total_procedures_year' not in grouped.columns:
                            grouped['total_procedures_year'] = 0
                        grouped['_robot_share'] = grouped['ROB'] / grouped['_approach_tot'] * 100.0 if 'ROB' in grouped.columns else 0.0
                        grouped['hid'] = grouped['hid'].astype(str)

                        # Split selected vs others
                        name_map = establishments.set_index('id')['name'].to_dict() if 'name' in establishments.columns else {}
                        grouped['name'] = grouped['hid'].map(lambda i: name_map.get(i, str(i)))
                        sel = grouped[grouped['hid'] == str(selected_hospital_id)]
                        others = grouped[grouped['hid'] != str(selected_hospital_id)]

                        fig_rs2 = go.Figure()
                        fig_rs2.add_trace(go.Scatter(
                            x=others['total_procedures_year'], y=others['_robot_share'], mode='markers',
                            marker=dict(color='#60a5fa', size=6, opacity=0.75), name='Other hospitals',
                            hovertemplate='%{text}<br>Procedures: %{x:.0f}<br>Robot share: %{y:.0f}%<extra></extra>', text=others['name']
                        ))
                        if not sel.empty:
                            fig_rs2.add_trace(go.Scatter(
                                x=sel['total_procedures_year'], y=sel['_robot_share'], mode='markers',
                                marker=dict(color='#FF8C00', size=12, line=dict(color='white', width=1)), name='Selected hospital',
                                hovertemplate='%{text}<br>Procedures: %{x:.0f}<br>Robot share: %{y:.0f}%<extra></extra>', text=sel['name']
                            ))
                        fig_rs2.update_layout(
                            height=360,
                            xaxis_title='Number of procedures per year (any approach)',
                            yaxis_title='Robot share (%)', yaxis=dict(range=[0,100]),
                            plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)'
                        )
