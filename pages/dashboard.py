...
st.markdown("---")
st.markdown("#### 📊 Export Hospital Data")

# Create export data
export_data = {
    'hospital_info': {
        'name': selected_hospital_details['name'],
        'city': selected_hospital_details['ville'],
        'status': selected_hospital_details['statut'],
        'total_surgeries': total_proc_hospital,
        'revision_surgeries': total_rev_hospital,
        'revision_percentage': hospital_revision_pct
    },
    'annual_data': selected_hospital_all_data.to_dict('records'),
    'bariatric_procedures': bariatric_summary.to_dict(),
    'surgical_approaches': approach_summary.to_dict()
}
...
with col1:
    if st.button("📄 Export Hospital Summary (CSV)", use_container_width=True):
        try:
            import pandas as pd
            from analytics_integration import track_data_export
            
            # Create summary dataframe
            summary_df = pd.DataFrame([export_data['hospital_info']])
            
            # Track export
            track_data_export("hospital_summary", 1, {
                "hospital_id": selected_hospital_id,
                "hospital_name": selected_hospital_details['name'],
                "export_type": "summary"
            })
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"hospital_summary_{selected_hospital_id}.csv",
                mime="text/csv"
            )
...
with col2:
    if st.button("📊 Export Annual Data (CSV)", use_container_width=True):
        try:
            import pandas as pd
            from analytics_integration import track_data_export
            
            # Track export
            track_data_export("hospital_annual_data", len(export_data['annual_data']), {
                "hospital_id": selected_hospital_id,
                "hospital_name": selected_hospital_details['name'],
                "export_type": "annual_data"
            })
            
            # Download
            csv = selected_hospital_all_data.to_csv(index=False)
            st.download_button(
                label="📥 Download Annual Data CSV",
                data=csv,
                file_name=f"hospital_annual_data_{selected_hospital_id}.csv",
                mime="text/csv"
            )
        except Exception as e:
            st.error(f"Export failed: {e}")
