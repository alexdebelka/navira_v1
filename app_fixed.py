...
import pandas as pd
from auth import init_session_state, login_page, register_page, user_dashboard
...
def admin_panel():
    ...
    with tab1:
        ...
        if not users_df.empty:
            ...
            if selected_user:
                user_row = users_df[users_df['username'] == selected_user]
                user_id = user_row['id'].iloc[0]
                user_email = user_row['email'].iloc[0]
                
                st.write(f"**Selected User:** {selected_user} ({user_email})")
